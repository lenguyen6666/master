<?php
class BgmenuController extends AppController {

	var $name = 'Bgmenu';
	var $helpers = array('Html', 'Form', 'Javascript', 'TvFck');
	function index() {
		  $this->account();		
		  $this->set('bgmenu', $this->Bgmenu->find('all',array('conditions'=>array(),'limit'=>1)));
	}
	//Them bai viet
	function add() {
		$this->account();
		if (!empty($this->data)) {
			$this->Bgmenu->create();
			$data['Bgmenu'] = $this->data['Bgmenu'];
			if ($this->Bgmenu->save($data['Bgmenu'])) {
	...
	function view($id = null) {
		...
		$this->set('views', $this->Bgmenu->read(null, $id));
	}
	//close tin tuc
	...
	function edit($id = null) {
		...
		if (empty($this->data)) {
			$this->data = $this->Bgmenu->read(null, $id);
		}        
		$this->set('edit',$this->Bgmenu->findById($id));
	}
	// Xoa cac dang
