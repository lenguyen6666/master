...
class BackgroundController extends AppController {
	...
	function index() {
		  $this->account();		
		  $this->set('background', $this->Background->find('all',array('conditions'=>array(),'limit'=>1)));
	}
	//Them bai viet
	function add() {
		$this->account();
		if (!empty($this->data)) {
			...
			$data['Background'] = $this->data['Background'];
			if ($this->Background->save($data['Background'])) {
	...
	function view($id = null) {
		...
		$this->set('views', $this->Background->read(null, $id));
	}
	//close tin tuc
